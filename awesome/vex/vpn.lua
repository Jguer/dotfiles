            ...
            function(stdout, _, _, _)
                ...
                for line in stdout:gmatch("[^\r\n]+") do
                    if line:match("%w*openvpn") and not line:match("sudo") then
                        file = line:sub(line:find("/[^/]*$") + 1):match("(%S+)%.ovpn")
                ...
                w:set_markup(
                    string.format("%s<span color='%s'>%s</span>", w._private.prefix, color, w._private.connected)
                )
                w._timer:again()
            end
        )
        ...
        gtable.join(
            button(
                ...
                function()
                    ...
                    if not k then
                        k = next(w._private.ovpns, nil)
                    end
                    w._private.selected = k
                    w:set_markup(
                        string.format("%s<span color='%s'>%s</span>", w._private.prefix, beautiful.widget.fg, k)
                    )
