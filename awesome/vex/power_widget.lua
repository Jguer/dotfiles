...
local function get_percentage()
    ...
    if percentage then return math.floor(percentage) end
...
local function update_icon(widget)
    ...
    if icon then widget.image = icon:load_surface() end
end
...
local function maybe_warn(widget, warning_condition, notification_preset,
    ...
    if warning_condition then
        local warning_level = device.warninglevel or
                                  power.enums.BatteryWarningLevel.None
        local msg = message or
                        (warning_level.name == "None" and "Low" or
                            warning_level.name) .. " battery!"
        ...
        if device.type == power.enums.DeviceType.Battery then
            ...
            if when then
                charge_status_msg = string.format("\n%s in %s", what,
                                                  to_hour_min_str(when))
            end
        end

        widget.tooltip:set_text(string.format("%d%% - %s%s", percentage,
                                              device.state.name,
                                              charge_status_msg))
    else
...
local function should_warn_critical(widget)
    if not device.IsPresent then return false end

    local percentage = get_percentage()

    return (device.state == power.enums.BatteryState.Discharging and
               (percentage <= widget.critical_percentage or device.warninglevel ==
                   WarningLevel.Low or device.warninglevel ==
...
local function update(widget)
    ...
    if not critical_warn then
        maybe_warn(widget, get_percentage() <= widget.warning_config.percentage,
                   widget.warning_config.preset, widget.warning_config.message)
    end
...
local function init(widget)
    ...
    device =
        power.create_device("/org/freedesktop/UPower/devices/DisplayDevice")

    device:on_properties_changed(function() update(widget) end)
