...
function hotkeys:init(args)
  ...
  local dropdown = quake({ app = "kitty", argname = "--class=%s", width=0.5,
  ...
  -- {{{ Global Keys
  -- {{{ General
  self.keys.root = gears.table.join(
    ...
    awful.key({ modkey, "Control" }, "n", restore_client,
  ...
  for i = 1, 9 do
    self.keys.root = awful.util.table.join(
      self.keys.root,
      tag_numkey(i,    { modkey },                     function(t) t:view_only()               end),
      tag_numkey(i,    { modkey, "Control" },          function(t) awful.tag.viewtoggle(t)     end),
      client_numkey(i, { modkey, "Shift" },            function(t) client.focus:move_to_tag(t) end),
      client_numkey(i, { modkey, "Control", "Shift" }, function(t) client.focus:toggle_tag(t)  end)
    )
  end
  -- }}}
  -- }}}
  ...
  -- {{{ Client Keys
  self.keys.client = gears.table.join(
    awful.key({ modkey,           }, "f",
      function (c)
